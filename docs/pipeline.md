...
A scene supported by the final page of a window may be saved for the next window. A scene cannot reveal beyond the final book page. The next plan explicitly carries it with `carryUntilPage`. Render jobs are created for spans intersecting the current page through twelve pages ahead, including scenes that started earlier and remain visible. Page changes atomically remove obsolete queued or failed render jobs and refresh priorities; returning to an unrendered scene schedules it again. The worker also checks the current window when claiming render jobs. Requests already running finish and save their images. Planning remains sequential through the highest requested target (current page plus 49, bounded by the book). Invalid optional carry-over endpoints are discarded with a structured trace warning; new scenes and character references still undergo full validation. Saved artifacts remain hidden outside their spans; returning to earlier pages reuses them.
